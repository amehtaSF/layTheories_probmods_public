...
var joinModel_mcmc_samples = 1e3
...
//     condition(situation == sit)
    return {
      ...
      arousal: arousal,
      anger: sample(inferredAngerDist), 
      sadness: sample(inferredSadnessDist),
      happiness: sample(inferredHappinessDist)
    }
...
var makeData = function(){
  ...
  return {situation: sit, anger: anger, happiness: happiness, sadness: sadness}
}
...
var layTheory = function(sit){
  return Infer({method: 'MCMC', samples: layTheory_mcmc_samples, burn: layTheory_mcmc_burn}, function(){
    ...
    var angerProb = mem(function(state) { return beta(1,1) })
    var happinessProb = mem(function(state) { return beta(1,1) })
    var sadnessProb = mem(function(state) { return beta(1,1) })
...
// // var x = layTheory('transgression')
// // var x = layTheory('accomplishment')
// print(expectation(marginalize(x, 'anger')))
// print(expectation(marginalize(x, 'sadness')))
// print(expectation(marginalize(x, 'guilt')))
// print(expectation(marginalize(x, 'happiness')))
// viz.marginals(x)
...
//     var egDistWeight = beta(1, 1)
    var egDistWeight = .5
    var ltDistWeight = 1 - egDistWeight 

    return {
      anger: sample(Mixture({dists: [marginalize(egDist, 'anger'), 
                                     marginalize(ltDist, 'anger')], 
                             ps: [egDistWeight, ltDistWeight]})),
      sadness: sample(Mixture({dists: [marginalize(egDist, 'sadness'), 
                                       marginalize(ltDist, 'sadness')], 
                               ps: [egDistWeight, ltDistWeight]})),
                               happiness: sample(Mixture({dists: [marginalize(egDist, 'happiness'), 
